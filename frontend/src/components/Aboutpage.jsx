...
const Aboutpage = ({ theme, setTheme }) => {
  return (
    <div className="bg-white text-gray-900 dark:bg-neutral-950 dark:text-white transition-colors duration-500 overflow-hidden">
      <Nav theme={theme} setTheme={setTheme} />

      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 text-white py-16 sm:py-20 px-4 sm:px-6 md:px-10 text-center overflow-hidden">
        ...
        >
          ...
          <p className="text-base sm:text-lg md:text-xl mb-8 max-w-2xl md:max-w-3xl mx-auto drop-shadow">
    ...
    Start Learning Now
          ...
          >
            <img
              ...
              className="mx-auto max-w-xs sm:max-w-md md:max-w-xl w-full h-auto drop-shadow-2xl rounded-2xl"
            />
          </motion.div>
        </motion.div>
      </section>
      ...
      >
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-10 sm:mb-12 text-indigo-500">
          How It Works
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 sm:gap-10 max-w-5xl mx-auto">
          {howItWorksSteps.map((step, index) => (
            <motion.div
              key={index}
              className="flex flex-col items-center p-6 sm:p-8 border border-indigo-100 dark:border-indigo-900 rounded-xl shadow-md hover:shadow-indigo-500/30 hover:-translate-y-2 transition-all duration-300 bg-white dark:bg-neutral-900"
              initial={{ opacity: 0, y: 30 }}
      ...
      {/* Features Section */}
      <motion.section
        className="bg-indigo-50 dark:bg-neutral-900 py-16 sm:py-20 px-4 sm:px-6 text-center"
        variants={sectionVariants}
        initial="hidden"
        whileInView="visible"
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <motion.div
              key={index}
              className="p-5 sm:p-6 rounded-xl bg-white dark:bg-neutral-800 shadow-md hover:shadow-indigo-500/30 transition-transform transform hover:-translate-y-2"
      ...
      >
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 text-indigo-500">
          Your Next Skill Upgrade is Just a Tap Away 🔑✨
        </h2>
        <p className="mb-8 max-w-xl sm:max-w-2xl mx-auto text-base sm:text-lg text-gray-600 dark:text-gray-300">
          Turn free knowledge into real-world expertise with TutorTube.
        </p>
        <motion.button
          className="bg-indigo-600 text-white font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-lg shadow-lg hover:bg-indigo-700 transition-transform transform hover:scale-105 text-sm sm:text-base"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        >
